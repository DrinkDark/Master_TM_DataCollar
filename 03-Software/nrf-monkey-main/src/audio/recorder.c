...
static volatile bool force_stop_recording;

bool is_saving_enable;
static int32_t recorder_sample_offset;
static volatile uint8_t recorder_store_idx;
static uint8_t store_tail_idx = 0;
...
struct k_poll_event rec_events[] = {
    K_POLL_EVENT_STATIC_INITIALIZER(K_POLL_TYPE_SEM_AVAILABLE,
                                    ...
                                    &recorder_toggle_transfer_sem, 0),
    K_POLL_EVENT_STATIC_INITIALIZER(K_POLL_TYPE_SEM_AVAILABLE,
                                    K_POLL_MODE_NOTIFY_ONLY,
                                    &recorder_toggle_saving_sem, 0),
};
    ...
    #if DT_NODE_HAS_STATUS(I2S_NODE, okay)
    {
        ...
        while (!must_be_in_power_saving_mode)
        {
            ...
            if (sdcard_is_ready()) 
            {
                ...
                // Reset pointers for new session
                recorder_store_idx = 0;
                store_tail_idx = 0;
                k_sem_reset(&recorder_file_access_sem);
                ...
                while (k_sem_take(&recorder_toggle_transfer_sem, K_NO_WAIT) != 0) {
                    ...
                    if (rec_events[1].state == K_POLL_STATE_SEM_AVAILABLE) {
                        ...
                        rec_events[1].state = K_POLL_STATE_NOT_READY;
                        ...
                        while (k_sem_take(&recorder_toggle_saving_sem, K_NO_WAIT) != 0 && !force_stop_recording) {
                            void *mem_block;
                            uint32_t block_size;
                            int ret;
...
void recorder_thread_store_to_file(void) 
{
    ...
    while (1) 
    {
        // Block here until signaled by producer. Allows sleep mode.
        k_sem_take(&recorder_file_access_sem, K_FOREVER);
        
        if (must_be_in_power_saving_mode) break;

        // Process buffers until Tail catches up to Head
        while (store_tail_idx != recorder_store_idx) {
            ...
            if(is_recorder_file_opened) {
                ...
                // Write the buffer at the tail position
                w_res = sdcard_write(&file, store_block[store_tail_idx], store_block_size);
                time(&end_t);

                if (w_res == store_block_size) {
                    store_tail_idx = (store_tail_idx + 1) & 0x01;
