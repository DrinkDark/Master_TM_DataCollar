...
bool sdcard_sd_detect_gpio_init(void)
{
    #if DT_NODE_HAS_STATUS(SD_CD0_NODE, okay)
    {
        ...
        static struct gpio_callback sd_detect_gpio_callback;
        ...
        if (sd_detect_gpio_init)
        {
            ...
            gpio_init_callback(&sd_detect_gpio_callback, sdcard_sd_detect_gpio_handler, BIT(card_detect_gpio.pin));
            gpio_add_callback(card_detect_gpio.port, &sd_detect_gpio_callback);
...
int sdcard_init(const char* pdrv) {
    int res = FR_OK;
    if (sdcard_sd_detect_gpio_init()) 
        LOG_INF("sdcard_sd_detect_gpio_init() Completed!");
    else
    {
        res = -EIO;
        LOG_WRN("sdcard_sd_detect_gpio_init() FAILED! error: %d", res);
    }
...
void sdcard_thread_fatfs_mount(struct fs_mount_t* mp_thread) 
{
    ...
    while (!is_sd_mic_gpio_set) {
        if (/*is_low_batt_detected ||*/ must_be_in_power_saving_mode) {
            ...
            LOG_WRN("------------ THREAD for FAT FS ended earlier! ------------");
            return;
        }
        k_msleep(2000);
    }
    ...
    char f_name[20];
    memset(f_name, 0x00, 20);
    sprintf(f_name, "%s/info.txt", mount_pt);
    ...
    do {
        ...
        k_sem_take(&sdcard_activity_sem, K_MSEC(2500));
        
        // Checking Button activity
        if (btn0_has_been_pressed()) {
            LOG_INF("listing the content of SD Card...");
            sdcard_lsdir(mount_pt);
        }

        if (btn1_has_been_pressed()) {
            LOG_DBG("Looking for \"info.txt\" file");
            if (sdcard_file_exists(f_name)) {
                LOG_INF("\"info.txt\" exists !");
            } else {
                LOG_WRN("\"info.txt\" not found on SD Card !");
            }
        }
        ...
        if (btn2_has_been_pressed()) {
            ...
            if (file_ready) {
                const char some_text[] = "here is some text.\n";
                const size_t text_len = strlen(some_text);
                size_t wr_size = sdcard_write(&file, some_text, text_len);
                LOG_DBG("written %d characters(s)...", wr_size);
                if (wr_size == text_len) {
                    (void) 0;
                }
